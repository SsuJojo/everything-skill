...
class WrapperFunctionTests(unittest.TestCase):
    ...
    def test_utf8_output_decodes_chinese_path(self):
        ...
        self.assertEqual(entries[0]['encoding'], 'utf-8')
...
class WrapperMainTests(unittest.TestCase):
    def run_main(self, argv, completed_process):
        ...
        with (
            ...
            mock.patch.object(
                es_wrapper.subprocess, 'run', return_value=completed_process
            ) as run,
    ...
    def test_json_preserves_native_exit_code_and_paging(self):
        ...
        self.assertFalse(payload['output_limited'])
        self.assertEqual(payload['next_offset'], 12)
        self.assertEqual(command[1:], ['query', '-max-results', '2', '-offset', '10'])

    def test_full_page_advances_and_empty_pages_stop_naturally(self):
