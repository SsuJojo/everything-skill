...
Assert-True (-not $helperText.Contains('C:\Users\SsuJo_')) 'Helper must not use the old workspace path.'
Assert-True (-not $helperText.Contains('voidtools/Everything')) 'Helper must use the ES release repository.'
Assert-True ($helperText.Contains('backup-es.exe')) 'Helper must retain a rollback copy during replacement.'
Assert-True ($helperText.Contains('AllowDownload')) 'Helper must require explicit download consent.'
